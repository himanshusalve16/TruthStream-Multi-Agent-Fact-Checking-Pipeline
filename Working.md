...
### 5. Adaptive Multi-Pipeline Routing Logic
TruthStream implements an adaptive, runtime-complexity-based classification and routing layer to choose the lightest viable path for speed and rate-limit conservation:
- **Fast Path**: Triggered for short raw text blocks (< 600 words). It runs `run_fast_path_pipeline_flow`, bypassing internet search and performing claim extraction, bias scoring, and verdict synthesis in a single unified Gemini model call. Time budget: **3.5s – 5.0s**.
- **Standard Path**: Triggered for standard URLs and medium articles. It runs `run_standard_path_pipeline_flow` which utilizes the **Scraper Bypass** mode. It formulates search queries but evaluates evidence and stances strictly using search snippets and domains (no full-page crawling). Time budget: **6.8s – 11.7s**.
- **Deep Path**: Triggered for complex articles, highly contested claims, or deep analyses. It runs `run_deep_path_pipeline_flow` which fetches full HTML text for the top 3 search results in parallel, cleans them via a background thread executor, and executes full stance evaluations on the scraped page body. Time budget: **12.3s – 26.7s**.
- **Recovery Path**: If any API key is exhausted or registry checks fail, it falls back to the recovery route (`run_recovery_pipeline_flow`) which generates a best-effort verdict from the raw text alone.
- **Queue Overload Downgrade**: If the queue delay for a job exceeds **15.0 seconds**, the system automatically downgrades the job complexity to `fast` and truncates the input text to 600 words to prevent client timeout and clear queue congestion.

### 6. Source-First Claim Verification Model
Verdicts are strictly driven by claim-level verification and external sources:
- **No AI-Only Reasoning**: The final verdict is based entirely on the corroboration scores calculated across the found external sources. If zero external sources can be verified for a claim, the system rejects AI-only speculation and marks the claim as `UNVERIFIABLE`.
- **Claim-Source Accordion Mapping**: Each extracted claim is mapped to its specific verification sources. Each source stores:
  - `title`: Title of the source web page.
  - `url`: Absolute web URL.
  - `domain`: Base domain authority (e.g. `bbc.com`).
  - `snippet`: Exact contextual excerpt retrieved from the search result.
  - `stance`: Classified stance label (`SUPPORT`, `CONTRADICT`, or `NEUTRAL`) generated by the stance-classification model.

### 7. Claim Caps & Source Limits
To maintain high responsiveness, bounded costs, and reliability:
- **Hard Claim Caps**: At most **5 claims** are processed per article (`MAX_CLAIMS` = 5). On the Fast Path, this is capped at **2 claims** (`MAX_CLAIMS_FAST` = 2). Under moderate rate pressure, the system drops the cap to **3 claims** (`MAX_CLAIMS_MODERATE` = 3) to protect Gemini quota.
- **Search Query Budgets**: Limits the maximum search queries per job: Fast/Recovery = 1 query, Standard = 2 queries, Deep = 3 queries.
- **Source Budgets**: At most **2 external sources** are mapped per claim. Search query execution stops early (`STOP_EARLY_THRESHOLD` = 1) as soon as at least 1 high-quality matching source is verified, minimizing redundant search and evaluation cost.

---

## F. Frontend Internal Working

The React frontend features a premium, responsive glassmorphic dark dashboard utilizing vibrant HSL colors and dynamic transitions to present verification results clearly.
...
JobPage
  ...
  ├── UI Layout Grid
        ├── LoadingState (Renders current stage and routing path: fast, standard, deep)
        ├── OrchestrationBoard & Live Feed (Visual progress of workers)
        ├── VerdictBanner (Displays overall verdict and confidence description)
        ├── D3 Visualizations (ConfidenceGauge and VerdictTimeline)
        └── ClaimList -> ClaimCard (Consensus Stream) -> SourceCard (Verification Sources Panel)
```

### 1. Key UI Components
- **Modern Dark AI Dashboard**: Utilizes a dark background with HSL-themed accent colors. All card interfaces employ a backdrop-blur glassmorphic design.
- **Live Pipeline Orchestration Board & Execution Feed**: Located in `LoadingState.tsx`. When a job is running, it renders the exact stage the pipeline is in (e.g. `fetching`, `extracting`, `routing`, `processing`, `done`). It displays real-time execution events detailing pipeline diagnostics and queue statuses.
- **Verdict Banner & Cards**: Shows the overall rating (e.g., `SUPPORTED`, `REFUTED`, `CONTESTED`, `UNVERIFIABLE`), a detailed summary, and the overall trust rating.
- **Claim Consensus Stream**: Displays individual claim cards containing the claim text, its specific veracity rating, and a breakdown of source stance counts.
- **Verification Sources Panel**: Nested within the claim cards, it lists all verified external sources showing the stance badge (supports, contradicts, neutral), source title, clickable URL, domain name, and snippet.
...
### 3. D3 gauge & Timeline Visualizations
